...
describe('Test FactomEventEmitter', () => {
    ...
    it('should add then remove a directory block listener', done => {
        ...
        const listener = dBlock => {
            ...
            assert.strictEqual(emitter.listenerCount('newDirectoryBlock'), 1);
    ...
    it('should add then remove factoid pending transaction address listener', done => {
        const emitter = new FactomEventEmitter(cli, { interval: INTERVAL });
        const pendingTransaction = {eventType: 'newPendingTransaction', address: 'FA29eyMVJaZ2tbGqJ3M49gANaXMXCjgfKcJGe5mx8p4iQFCvFDAC'}
        const tokenizedPendingTransaction = FactomEventEmitter.getSubscriptionToken(pendingTransaction)

        const listener = tx => {
            assert.isTrue(emitter.isPolling)
            assert.lengthOf(emitter.listeners(tokenizedPendingTransaction), 1);
            assert.strictEqual(tx[0].transactionid, '337a32712f14c5df0b57a64bd6c321a043081688ecd4f33fd8319470da2256b1')
